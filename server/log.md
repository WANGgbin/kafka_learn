...
# 零拷贝
kafka 中的零拷贝指的是什么? 有什么用?

这涉及 `sendfile()` 系统调用, 该调用直接将数据从一个描述符传输到另一个描述符, 避免一些额外的内存拷贝, 从而提高数据传输的效率. 关于 sendfile 的详细描述可以参考 linux_learn.

在 kafka 中, 经常需要从磁盘中读取消息发送给消费者, 这里就是通过 sendfile 来提高性能的.

与 sendfile 类似的还有 splice(粘接),这两有什么区别呢?
- sendfile 只能将数据从一个文件描述符传输到套接字, 但是 splice 可以在任意两个描述符之间传输数据
- 实际上, 新版的 sendfile 的底层也是通过 splice 实现的.